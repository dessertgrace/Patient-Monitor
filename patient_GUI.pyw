...
#  def create_output(record, name="", heart_rate = 1, medical_image="", ECG_image=""):
    #  out_string = "Patient name: {}\n".format(name)
    #  out_string += "Medical Record Number: {}\n".format(record)
    #  out_string += "Heart Rate": 
    #  answer = add_new_info_to_server(record, name=name, heart_rate=heart_rate, medical_image=medical_image, ECG_image=ECG_image)
...
def design_window():
    ...
    def select_img_cmd():
        ...
        and choosing an image from the local computer (with the
        initial directory set to "images" to make this process
        quicker), the image file will be opened, loaded and resized,
    ...
    def select_ECG_cmd():
        ...
        and choosing a file from the local computer (with the
        initial directory set to "test_data" to make this process
        quicker), the file will be opened and get_ECG_data will be
        called to determine the mean heart rate and JPG filename.
        The image is then resized and the label will be changed
        to reflect the updated image. The heart rate label on the
        GUI will also be updated to reflect the selected patient's
        heart rate.
        ...
        global img_string
        global img_string2
        ECG_file = filedialog.askopenfilename(initialdir="test_data")
    ...
    ttk.Label(root, text="Name:").grid(column=0, row=2, sticky='e', pady=10, padx=5)
